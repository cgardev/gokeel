...
- `conf` — externalized configuration from JSON documents with environment
  placeholders and JSON Schema generation. Its `go.mod` also has no `require`
  directive.
...
```sh
for module in transaction eventbus logging conf outbox outbox/gowaymigrator transaction/integration; do
  go -C "$module" build ./...
  go -C "$module" vet ./...
...
- Document every exported identifier with a complete sentence that begins with
  the identifier name.
- Keep the `transaction`, `eventbus`, `logging`, and `conf` modules free of
