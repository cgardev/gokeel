...
// The interface is structurally identical to the outbox Serializer, so one
// serializer instance can serve both modules when an application uses them
// side by side. The implementation is deliberately duplicated instead of
// imported: the modules are versioned independently and neither may depend
// on the other for its core contract.
...
// deserialize stored messages into the wrong type.
