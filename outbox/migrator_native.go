...
// scripts use CREATE TABLE / CREATE INDEX IF NOT EXISTS, so re-running them is a
// no-op; the native path therefore keeps no schema-history table of its own. The
// dialect is accepted for interface symmetry but is not needed here, because the
// IF NOT EXISTS DDL is portable across SQLite and PostgreSQL.
...
// semicolon boundary, discarding blank statements. The outbox migration scripts
// contain only plain DDL with no semicolons inside string literals or bodies, so
// a straight split is correct here. This is intentionally simple: the native
// path owns a fixed, package-controlled set of scripts, not arbitrary user SQL.
