...
// DialectSQLite and DialectPostgres, set by the respective store constructors. A
// Migrator adapter switches on it to choose its own dialect representation; for
// example the goway-backed adapter maps it to goway.SQLite() / goway.Postgres().
type Dialect string
...
// Migrator brings the outbox schema up to date against an open database. It is
// the single seam the store uses for schema initialization, so the choice of
// migration engine stays out of the query code. The store calls Migrate once,
// from Store.Initialize.
//
// Implementations receive the open *sql.DB (a concrete handle, because some
// engines require one rather than the narrower Querier), the target Dialect, and
// the embedded schema scripts as an fs.FS rooted at the directory that holds the
// SQL files, so an adapter never duplicates the SQL.
//
...
// zero-configuration default, so the common case needs no Migrator and the core
// go.mod carries no migration-engine dependency. A goway-backed implementation
// lives in its own module (github.com/cgardev/gokeel/outbox/gowaymigrator) so
// that only clients who opt in pull goway into their build.
