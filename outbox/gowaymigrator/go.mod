module github.com/cgardev/gokeel/outbox/gowaymigrator
...
// directives are NOT transitive to a consumer: outbox/go.mod's own replaces for
...
// outbox/gowaymigrator/, so outbox is one level up and the leaves are two.
