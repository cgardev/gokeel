...
# the tags transaction/v0.3.0, eventbus/v0.3.0, logging/v0.3.0, conf/v0.3.0,
# outbox/v0.3.0, outbox/gowaymigrator/v0.3.0, sqlbus/v0.3.0 and
# sqlbus/gowaymigrator/v0.3.0.
#
...
echo "==> tagging leaves transaction/$V, eventbus/$V, logging/$V and conf/$V"
git tag "transaction/$V"
git tag "eventbus/$V"
git tag "logging/$V"
git tag "conf/$V"
git push "$remote" "transaction/$V" "eventbus/$V" "logging/$V" "conf/$V"
...
echo "==> released gokeel $V (transaction/$V, eventbus/$V, logging/$V, conf/$V, outbox/$V, outbox/gowaymigrator/$V, sqlbus/$V, sqlbus/gowaymigrator/$V)"
